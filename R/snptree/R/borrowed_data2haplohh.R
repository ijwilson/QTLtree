## This is my edit of the data2haplohh file from the rehh library
## I am just looking at it to get some idea of exactly what it is doing
## I will probably just use the library, although the readfastphase bit of it
## looks quite interesting
#
# The code is not to my taste as the functions are far too long.

setClass(Class = "haplohh",
		representation(haplo = "matrix",
		               position = "numeric",
					   snp.name="character",
					   chr.name = "character",
					   nhap = "numeric",
...
read_map_file <- function(map_file) {
	snp_map <- read.table(map_file, row.names=1, colClasses="character")
	...
	if (length(tmp_chr)!=1 & is.na(chr.name)) {
		...
		stop("please repeat with single chromosome")
	}
}
...
data2haplohh<-function(hap_file, map_file, min_maf=0, min_perc_geno.hap=100,
	...
	if (min_perc_geno.snp<0 | min_perc_geno.snp>100) {
		stop("The value for the argument min_perc_geno.snp should lie between 0 and 100")
	}
	## Read and verify the map file
	read.table(map_file, row.names=1, colClasses="character")
	# snp name, chromosome, position, allele ancestral, allele derive
	if (ncol(map)!=4) {
		cat("Wrong format for map data file: ", map_file,
	...
	if (haplotype.in.columns) {
		...
		if (tmp.nsnp!=res@nsnp){
			...
			## see if we have fastPHASE output
			...
			if (length(test_fphase_1)>0 & length(test_fphase_2)>0) { #fastphase file
				cat("Looks like a FastPHASE haplotype file\n")
				out_fphase<-scan(hap_file,what="character",sep="\n",quiet=TRUE)
				first_genotype = grep("BEGIN GENOTYPES",out_fphase)[1] + 1
				last_genotype = grep("END GENOTYPES",out_fphase)[1] - 1
				out_fphase <- out_fphase[first_genotype:last_genotype]
				...
				if (length(test_poplabel)>1){
					...
					population_list=unique(pop_label)
					cat("Haplotypes originate from ",length(population_list)," different populations in the fastPhase output file\n")
					...
					if (is.na(popsel)) { #on demande interactivement la pop a selectionner
						repeat {
							cat('Which population should be considered among:',population_list,"\n")
				...
				for (i in 1:nind) {
					...
					if (length(hap2)!=res@nsnp){
						...
						tmp_haplos[hap2_index,]=hap2}
					}
				} else { ## file in a standard format
